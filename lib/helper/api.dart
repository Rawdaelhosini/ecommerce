...
class Api
{
  ...
  Future<dynamic> post({required String url, @required dynamic body,@required String? token}) async
  {
    ...
    http.Response response  = await http.post(Uri.parse(url), body:body, headers: heeaders);
    if(response.statusCode == 200)
    {
  ...
  Future<dynamic> put({required String url, @required dynamic body,@required String? token}) async // بستخدمها عشان بتجدد البيانات الللي عندي
  {
    Map<String , String> headers = {};
    headers.addAll({
